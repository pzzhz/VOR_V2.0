...
#include "usbd_cdc_mydesc1.h"
#include "usbd_desc.h"
#include "usbd_req.h"
#include "../USB_APP/usb_std.h"
...
#define USBD_CDC_3CH_CONFIG_DESC_SIZE 207

__ALIGN_BEGIN static uint8_t usbd_cdc_CfgDesc[USB_CDC_CONFIG_DESC_SIZ] __ALIGN_END =
    {
        USB_DEVICE_CFG_DESC_LEN,          /* bLength: configuration descriptor size */
        USB_DESCIPTOR_TYPE_CONFIGURATION, /* bDescriptorType: configuration */
        LBYTE(USB_CDC_CONFIG_DESC_SIZ),   /* wTotalLength: bytes returned */
        HBYTE(USB_CDC_CONFIG_DESC_SIZ),   /* wTotalLength: bytes returned */
        ...
        CDC_CMD_EP,                  /* bEndpointAddress: the address of endpoint on usb device described by this descriptor */
        USB_EPT_DESC_INTERRUPT,      /* bmAttributes: endpoint attributes */
        LBYTE(8),
        HBYTE(8), /* wMaxPacketSize: maximum packe size this endpoint */
        0xff,     /* bInterval: interval for polling endpoint for data transfers */
        ...
        USB_DEVICE_EPT_LEN,          /* bLength: size of endpoint descriptor in bytes */
        USB_DESCIPTOR_TYPE_ENDPOINT, /* bDescriptorType: endpoint descriptor type */
        CDC_OUT_EP,                  /* bEndpointAddress: the address of endpoint on usb device described by this descriptor */
        USB_EPT_DESC_BULK,           /* bmAttributes: endpoint attributes */
        LBYTE(64),
        HBYTE(64), /* wMaxPacketSize: maximum packe size this endpoint */
        0x00,      /* bInterval: interval for polling endpoint for data transfers */

        USB_DEVICE_EPT_LEN,          /* bLength: size of endpoint descriptor in bytes */
        USB_DESCIPTOR_TYPE_ENDPOINT, /* bDescriptorType: endpoint descriptor type */
        CDC_IN_EP,                   /* bEndpointAddress: the address of endpoint on usb device described by this descriptor */
        USB_EPT_DESC_BULK,           /* bmAttributes: endpoint attributes */
        LBYTE(64),
        HBYTE(64), /* wMaxPacketSize: maximum packe size this endpoint */
        0x00,      /* bInterval: interval for polling endpoint for data transfers */
...
__ALIGN_BEGIN uint8_t usbd_cdc_CfgDesc2[USB_CDC_CONFIG_DESC_SIZ] __ALIGN_END =
    {
                              ...
                              USB_SETUP_REQ *req)
{
  ...
  case USB_REQ_TYPE_CLASS:
    ...
    if (req->wLength)
    {
      ...
      {
        uint8_t *buff = CmdBuff;

        /* Get the data to be sent to Host from interface layer */
        // APP_FOPS.pIf_Ctrl(req->bRequest, CmdBuff, req->wLength);
        buff[0] = (uint8_t)bitrate;
        buff[1] = (uint8_t)(bitrate >> 8);
        buff[2] = (uint8_t)(bitrate >> 16);
        buff[3] = (uint8_t)(bitrate >> 24);
        buff[4] = (uint8_t)(format);
        buff[5] = (uint8_t)(parity);
        buff[6] = (uint8_t)(data2);
...
void usbd_cdc_senddata(void *pdev, uint8_t *str, uint16_t len)
{
  DCD_EP_Tx(pdev,
            CDC_IN_EP,
            (uint8_t *)str,
