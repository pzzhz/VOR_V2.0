...
u8 GT9147_Init(void)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOG | RCC_AHB1Periph_GPIOB, ENABLE); // 使能GPIOB,C时钟
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5;		   // PC13设置为推挽输出
...
u8 GT9147_Online(void)
{
	return isGt9147;
}
static u16 GT9147_Original[3];
...
int offset=50;
u8 GT9147_GET_Touch( unsigned short Pos_data[3])
{
	if(isGt9147==0)
		return 0;
	GT9147_Scan(1);
	if(GT9147_Original[2]==1)
	{
		Pos_data[1]=GT9147_Original[0];
		Pos_data[0]=GT9147_Original[1];
			Pos_data[2]=1;
	}else
	{
		Pos_data[0]=0xffff;
