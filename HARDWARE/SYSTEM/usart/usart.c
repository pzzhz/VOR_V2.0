...
{
    static u8 usart_buf[256];
    u16 i = 0;
    memset(usart_buf, 0, 256);

    va_list ap;
    va_start(ap, fmt);
    vsprintf(usart_buf, fmt, ap);
    va_end(ap);
...
u16 My_Scanf(const char *fmt, ...)
{
    ...
    va_start(ap, fmt);
#if u2
    i = vsscanf(usart_buf, fmt, ap);
    memset(usart_buf, 0, 256);
#else
    i = vsscanf(USART_RX_BUF, fmt, ap);
