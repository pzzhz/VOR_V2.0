...
void ui1_(Table_Property* p)
{
	...
	if (p->Updata_Source)
	{
		...
		char* mode_Name = mode_name_array[task_info->mode];
...
void Control_btn_communication(uint8_t message[50], uint8_t* cmd)
{
	char* CMD = cmd;
...
void UI_Task_Btn_ADD_Click_Event(lv_event_t* e)
{
	...
	//设置好要写入的参数
	while (Control_Add_Task.flag == Flag_required);
...
void UI_Task_Btn_Del_Click_Event(lv_event_t* e)
{
	...
	//设置好要写入的参数
	while (Control_del_Task.flag == Flag_required);
	if (Control_del_Task.flag == Flag_Error)
	{
		LV_LOG_USER(" error %d", fouces_index);
		return;
	}
	UI_Table_Clean(table_Contain_Property->list, fouces_index, parent_bo2);
}
